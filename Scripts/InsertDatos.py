...
nombres_comuna = ["Santiago", "Providencia", "Las Condes", "La Reina",
                  "Vitacura", "Lo Barnechea", "Ñuñoa", "Macul",
...
#SERVICIOS
dict_servicio = {
    1: {'nombre': 'Corte de Pelo', 'descripcion': 'Corte a la moda', 'precio_servicio': 5000.0, 'duracion': 30.0},
    2: {'nombre': 'Manicure', 'descripcion': 'Cuidado de uñas', 'precio_servicio': 7000.0, 'duracion': 45.0},
    3: {'nombre': 'Masaje', 'descripcion': 'Masaje relajante', 'precio_servicio': 10000.0, 'duracion': 60.0},
    4: {'nombre': 'Depilacion', 'descripcion': 'Depilación completa', 'precio_servicio': 8000.0, 'duracion': 40.0},
    5: {'nombre': 'Pedicure', 'descripcion': 'Cuidado de pies', 'precio_servicio': 6000.0, 'duracion': 50.0},
    6: {'nombre': 'Tintura de Cabello', 'descripcion': 'Cambio de color', 'precio_servicio': 15000.0, 'duracion': 60.0},
    7: {'nombre': 'Facial', 'descripcion': 'Limpieza profunda', 'precio_servicio': 12000.0, 'duracion': 60.0},
    8: {'nombre': 'Peinado', 'descripcion': 'Estilo especial', 'precio_servicio': 4000.0, 'duracion': 30.0},
    9: {'nombre': 'Maquillaje', 'descripcion': 'Maquillaje profesional', 'precio_servicio': 10000.0, 'duracion': 60.0},
    10: {'nombre': 'Corte barba', 'descripcion': 'Perfilado perfecto', 'precio_servicio': 20000.0, 'duracion': 15.0}
...
for id_sede in dict_sede:
    for id_producto in dict_producto:
        dict_sede_producto[id_sp] = {
            'id_sede': id_sede,
            'id_producto': id_producto,
            'stock': random.randint(0, 100)
        }
        id_sp += 1
print("se generaron", len(dict_sede_producto), " = sedesxproductos")     
...
for id_sede in dict_sede:
    for id_servicio in dict_servicio:
        dict_sede_servicio[id_ss] = {
            'id_sede': id_sede,
            'id_servicio': id_servicio
        }
        id_ss += 1
print("se generaron", len(dict_sede_servicio), " = serviciosxsede")
...
for id_venta in range(1, (VENTAS_POR_SEDE*10) +1):
    ...
    for _ in range(0,random.randint(1, MAX_PRODUCTOS_POR_VENTA+1)):
        id_producto = random.choice(list(dict_producto.keys()))
        cantidad = random.randint(1, 3)
        total += dict_producto[id_producto]['precio_venta']*cantidad
...
for id_cita in dict_cita:
    ...
    if(dict_cita[id_cita]['estado'] == PENDIENTE):
        fecha = None
        monto = dict_servicio[dict_cita[id_cita]['id_servicio']]['precio_servicio']
        estado = PENDIENTE
    elif(dict_cita[id_cita]['estado'] == ANULADA):
        fecha = None
        monto = 0
        estado = ANULADA
    elif(dict_cita[id_cita]['estado'] == REALIZADA):
...
try:
    ...
    BorrarBDD.reset_database('peluqueria', 'postgres', 'admin', '5432', 'localhost', 'Create.sql')
    ...
    with conn:
        ...
        with conn.cursor() as cur:
            for id_pago in dict_pago:
                cur.execute("INSERT INTO pago (fecha, monto, estado) VALUES (%s, %s, %s)",
                            (dict_pago[id_pago]['fecha'], dict_pago[id_pago]['monto'], dict_pago[id_pago]['estado']))
        conn.commit()
        ...
        #INSERTAR SEDE_PRODUCTO
        with conn.cursor() as cur:
            for id_sp in dict_sede_producto:
                cur.execute("INSERT INTO sede_producto (id_sede, id_producto, stock) VALUES (%s, %s, %s)",
                            (dict_sede_producto[id_sp]['id_sede'], dict_sede_producto[id_sp]['id_producto'], dict_sede_producto[id_sp]['stock']))
        conn.commit()
        ...
        with conn.cursor() as cur:
            for id_servicio in dict_servicio:
                cur.execute("INSERT INTO servicio (nombre, descripcion, precio_servicio, duracion) VALUES (%s, %s,%s, %s)",
                            (dict_servicio[id_servicio]['nombre'], dict_servicio[id_servicio]['descripcion'], dict_servicio[id_servicio]['precio_servicio'], dict_servicio[id_servicio]['duracion']))
        conn.commit()
        print("Se insertaron los servicios correctamente")
        
        #INSERTAR SEDE_SERVICIO
        with conn.cursor() as cur:
            for id_sp in dict_sede_servicio:
                cur.execute("INSERT INTO sede_servicio (id_sede, id_servicio) VALUES (%s, %s)",
                            (dict_sede_servicio[id_sp]['id_sede'], dict_sede_servicio[id_sp]['id_servicio']))
        conn.commit()
        print("Se insertaron los servicios por sede correctamente")
        ...
        with conn.cursor() as cur:
            for id_venta_producto in dict_venta_producto:
                cur.execute("INSERT INTO venta_producto (id_venta, id_producto, cantidad) VALUES (%s, %s, %s)",
                            (dict_venta_producto[id_venta_producto]['id_venta'], dict_venta_producto[id_venta_producto]['id_producto'], dict_venta_producto[id_venta_producto]['cantidad']))
        conn.commit()
