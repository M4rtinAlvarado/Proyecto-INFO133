...
for id_sede in dict_sede:
    ...
    for id_cargo in nombres_cargo:
        if id_cargo != 1:
            if(random.choices([True, False], weights=[0.8, 0.2], k=1)[0]):#chance de que se generar empleados de ese cargo
...
for id_empleado in dict_empleado:
    if(dict_empleado[id_empleado]['id_cargo'] != 1):
        for i in range(0, random.randint(365, 365*4)):
            ...
            dict_citas_empleado[id_empleado].append(fecha_hora)
...
if conn is not None:
    try:
        ...
        BorrarBDD.reset_database(DB_NAME_1, DB_USER_1, DB_PASSWORD_1, DB_PORT_1, DB_HOST_1, 'CreateTablesDBT.sql')
